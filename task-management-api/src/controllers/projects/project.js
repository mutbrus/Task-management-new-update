...
exports.delete = async (req, res) => {
    const { pj_hash } = req.params;
    const userId = req.user_id;

    await req.db.beginTransaction();
    try {
        const [proRow] = await req.db.query('SELECT owner_id FROM projects WHERE project_id = ? FOR UPDATE;', [pj_hash]);
        ...
        await req.db.query('DELETE FROM tasks WHERE project_id = ?', [pj_hash]);
        await req.db.query('DELETE FROM activity_log WHERE project_id = ?', [pj_hash]);
        await req.db.query('DELETE FROM project_members WHERE project_id = ?', [pj_hash]);
        await req.db.commit();
