...
exports.getSingle = async (req, res) => {
    ...
    try {
        ...
        if (taskRows.length === 0) return res.json(req.f.json.fail('Task not found in this project.'));
...
exports.create = async (req, res) => {
    ...
    try {
        if (parent_task_id) {
            const [parentTaskRow] = await req.db.query('SELECT project_id FROM tasks WHERE task_id = ?', [parent_task_id]);
            ...
            if (parentTaskRow[0].project_id.toString() !== project_id.toString()) {
                await req.db.rollback();
                return res.json(req.f.json.fail('Parent task must belong to the same project.'));
            }
        }
        ...
        if (assigneeIds && assigneeIds.length > 0) {
            ...
            const assigneeValues = assigneeIds.map(userId => [newTaskId, userId]);

            await req.db.query(assigneeSql, [assigneeValues]);
        }

        await req.db.commit();
        const [newTaskRows] = await req.db.query('SELECT * FROM tasks WHERE task_hash = ?', [task_hash]);
...
exports.update = async (req, res) => {
    ...
    if(Object.keys(req.body).length === 0) {
        return res.json(req.f.json.fail('Request body cannot be empty for an update.'));
    }

    await req.db.beginTransaction();
    try {
        const [memberRows] = await req.db.query('SELECT role FROM project_members WHERE user_id = ? AND project_id = ?', [userId, projectId]);
        const [assigneeRows] = await req.db.query('SELECT user_id FROM task_assignees WHERE task_id = ?', [taskId]);

        if (memberRows.length === 0) {
            await req.db.rollback();
            return res.json(req.f.json.fail(403, 'Access Denied: You are not a member of this project.'));
        }

        const userProjectRole = memberRows[0].role;
        const isAssigned = assigneeRows.some(assignee => assignee.user_id === userId);

        if (userProjectRole !== 'manager' && !isAssigned) {
            await req.db.rollback();
            return res.json(req.f.json.fail(403, 'Permission Denied: You must be a project manager or assigned to this task to update it.'));
        }
        ...
        if (assigneeIds && Array.isArray(assigneeIds)) {
            await req.db.query('DELETE FROM task_assignee WHERE task_id = ?', [taskId]);

            if (assigneeIds.length > 0) {
                const assigneeSql = 'INSERT INTO task_assignees (task_id, user_id) VALUES ?';
                const assigneeValues = assigneeIds.map(id => [taskId, id]);
                await req.db.query(assigneeSql, [assigneeValues]);
            }
        }

        await req.db.commit();
        const [updatedTaskRows] = await req.db.query('SELECT * FROM tasks WHERE task_id = ?', [taskId]);
        const [updatedAssigneeRows] = await req.db.query('SELECT user_id FROM task_assignees WHERE task_id = ?', [taskId]);
...
exports.delete = async (req, res) => {
    ...
    try {
        ...
        await req.db.query('DELETE FROM task_assignees WHERE task_id = ?', [taskId]);
        await req.db.query('DELETE FROM comments WHERE task_id = ?', [taskId]);

        const [deleteResult] = await req.db.query('DELETE FROM tasks WHERE task_id = ? AND project_id = ?', [taskId, projectId]);

        await req.db.commit();
        if (deleteResult.length === 0) return res.json(req.f.json.fail('Task not found in this project.'));
...
exports.updateStatus = async (req, res) => {
    ...
    if (status_id === undefined || typeof status_id !== 'number') {
        return res.json(req.f.json.fail('A numeric status ID is required in the request body.'));
    }
    ...
    try {
        ...
        if (userProjectRole !== 'manager' && !isAssigned) {
            return res.json(req.f.json.fail('Permission Denied: You must be a project manager or assigned to this task to update its status.'));
        }

        const sql = 'UPDATE tasks SET status_id = ?, updated_at = NOW() WHERE task_id = ? AND project_id = ?;';
